...
*/
	...
	if(!$result || ($num_rows < 0)){ retornar ("Error mostrando la información");}
	if($num_rows == 0){ retornar ("¡No hay un recurso con ese Id.!");}

	header("Content-Type: " . mysql_result($result,0,"mime"));
	$archivo = "../img/".$_GET['id'];
	if (file_exists($archivo)) echo file_get_contents($archivo); else echo "woops!, final stage!";
?>
